...
import { IconUploadApiRequest, SignUpApiResponse } from '@/types/user';
...
function SignUp() {
  ...
  const onSubmit = async (data: SignUpForm) => {
    ...
    const request: SignUpApiRequest = {
      name: data.name,
      email: data.email,
    ...
    setSuccess(true);
    const successResponse = response as SignUpApiResponse;
    const token = successResponse.token;
    ...
    const iconUploadResponse = await iconUpload(iconUploadRequest, token);
    if ("error" in iconUploadResponse && iconUploadResponse.error) {
      setSuccess(false);
      setError(iconUploadResponse);
      return;
    }
    navigate('/');
  }
